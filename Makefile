...
build-impl-%: build_%/Makefile
	@cmake --build build_$* -j $(NPROCS) --target eng_learning

# test
test-impl-%: build-impl-%
	@cmake --build build_$* -j $(NPROCS) --target eng_learning_unittest
	@cmake --build build_$* -j $(NPROCS) --target eng_learning_benchmark
	@cd build_$* && ((test -t 1 && GTEST_COLOR=1 PYTEST_ADDOPTS="--color=yes" ctest -V) || ctest -V)
	@pep8 tests

# testsuite service runner
service-impl-start-%: build-impl-%
	@cd ./build_$* && $(MAKE) start-eng_learning
...
install: build-release
	@cd build_release && \
		cmake --install . -v --component eng_learning
...
--debug-start-in-docker: install-debug
	@/home/user/.local/bin/eng_learning \
		--config /home/user/.local/etc/eng_learning/static_config.yaml
...
docker-start-service:
	@docker-compose run -p 8080:8080 --rm eng_learning make -- --debug-start-in-docker

# Build and runs service in docker environment
docker-start-service-debug:
	@docker-compose run -p 8080:8080 --rm eng_learning make -- --debug-start-in-docker-debug

# Start targets makefile in docker environment
docker-impl-%:
	docker-compose run --rm eng_learning make $*
